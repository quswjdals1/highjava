...
public class MemberController {
	...
	private void updateMember3() {
		...
		if(!tel.equals("")) {
			memberVO.setMemTel(tel);
		}
		System.out.println("mem_addr 수정값 입력: ");
		String addr = sc.nextLine();
		if(!addr.equals("")) {
			memberVO.setMemAddr(addr);
		}
	...
	private void updateMember2() {
		...
		case 1:
			...
			val=sc.nextLine();
			memberVO.setMemPass(val);
			break;
		case 2:
			sel="mem_name";
			System.out.println("수정할 이름 입력: ");
			val=sc.nextLine();
			memberVO.setMemName(val);
			break;
		case 3:
			sel="mem_tel";
			System.out.println("수정할 전화번호 입력: ");
			val=sc.nextLine();
			memberVO.setMemTel(sel);
			break;
		case 4:
			sel="mem_addr";
			System.out.println("수정할 주소 입력: ");
			val=sc.nextLine();
			memberVO.setMemAddr(val);
			break;
		default:
	...
	private void getAllMember() {
		...
		for (MemberVO memberVO : list) {
			System.out.println(memberVO.getMemId()+"\t"+memberVO.getMemPass()+"\t"+memberVO.getMemName()+"\t"+memberVO.getMemTel()+"\t"+memberVO.getMemAddr()+"\t");
		}
		System.out.println("-------------------------------");
	}
